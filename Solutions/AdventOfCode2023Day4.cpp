...
you're not climbing a mountain. As the circle of Snow Island recedes below you, an entire new landmass suddenly appears above you! 
...
We really need to come up with a better naming scheme. Tell you what: if you can help me with something quick, I'll let you borrow my boat and 
you can go visit the gardener. I got all these scratchcards as a gift, but I can't figure out what I've won."

The Elf leads you over to the pile of colorful cards. There, you discover dozens of scratchcards, 
all with their opaque covering already scratched off. Picking one up, it looks like each card has two lists of 
numbers separated by a vertical bar (|): a list of winning numbers and then a list of numbers you have. 
...
int sumWinningNumbers()
{
    std::ifstream file;
    file.open("AdventOfCode2023Day4.txt");
    ...
    {       
        ...
        while (std::getline(file, line))
        {
            ...
            std::string winningNumbers = line.substr(10, 41-11);
            std::string myNumbers = line.substr(42, line.length() -1);

            matchRegex(winningNumbers, std::regex("[0-9]+"), winningNumbersVec);
            matchRegex(myNumbers, std::regex("[0-9]+"), myNumbersVec);            

            std::vector<int> myWins;
            for (int i = 0; i < myNumbersVec.size(); i++)
            {                
                for (int j = 0; j < winningNumbersVec.size(); j++)
                {
                    if (myNumbersVec[i] == winningNumbersVec[j])
                    {
                     
                        myWins.push_back(myNumbersVec[i]);
                        break;
                    }
                }                
            }
...
int main()
{
    int sum = sumWinningNumbers();
    std::cout << "Sum Part 1: " << sum << std::endl;
}
