...
multiplying each hand's bid with its rank (765 * 1 + 220 * 2 + 28 * 3 + 684 * 4 + 483 * 5). So the total winnings in this example are 6440.
...
Hands determineHand(std::string& cards)
{

    auto FiveOfAKind = [cards](const char c) -> bool {
    ...
    auto FourOfAKind = [cards](const char c) -> bool {
        std::unordered_map<char, int> m;
    ...
    auto ThreeKind = [cards](const char c) -> bool {
        std::unordered_map<char, int> m;
